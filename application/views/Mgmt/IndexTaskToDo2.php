							...
							<?php for ($i = 0; $i < count($whatsnew); ++$i) { ?>
								...
								<?php endif;?>
									<div class="timeline-badge"><i class="glyphicon glyphicon-ok"></i></div>
										...
										<?php } ?>
										<!--Notice-->
										<?php elseif($whatsnew[$i]->NoticeID != ''): {?>
										<div class="timeline-panel panel-red">
											<div class="timeline-heading">
												<h4 class="timeline-title"><?php echo $whatsnew[$i]->NoticeSummary; ?></h4>
												<p><small class="text-muted"><i class="glyphicon glyphicon-time"></i>&nbsp;
													<?php echo date("d-m-Y h:i A", strtotime($whatsnew[$i]->CreatedDate)); ?>
												</small></p>
											</div>
											<div class="timeline-body">
												<p><?php echo $whatsnew[$i]->NoticeTitle; ?></p>
												<p><a href="<?php echo base_url()."index.php/Common/Notice/Detail/".$whatsnew[$i]->NoticeID;?>">View Details</a></p>
											</div>
										<?php } ?>
										<?php endif;?>
									</div>
