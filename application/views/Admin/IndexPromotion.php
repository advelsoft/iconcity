					...
					<div class="row">
						<?php for ($i = 0; $i < count($promoList); ++$i) { ?>
							<div class="col-lg-4 col-md-4 col-sm-6 col-xs-12 thumbimg">
								...
								<?php { echo '<a class="thumbnail wrapper" href="'.$promoList[$i]->PromoUrl.'='.$promoList[$i]->PromoCode.'" target="_blank">
											 <div class="image">'.$promoList[$i]->Summary.'</div></a>'; } ?>
								<?php elseif($promoList[$i]->PromoUrl != ''): ?>
								<?php { echo '<a class="thumbnail wrapper" href="'.$promoList[$i]->PromoUrl.'" target="_blank">
											 <div class="image">'.$promoList[$i]->Summary.'</div></a>'; } ?>
								<?php else: ?>
								<?php { echo '<a class="thumbnail wrapper"><div class="image">'.$promoList[$i]->Summary.'</div></a>'; } ?>
								<?php endif;?>
								<div class="promoText">
