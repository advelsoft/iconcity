							...
							<?php if ($promoList[$i]->Display == '1'): ?>
								...
								<?php if ($promoList[$i]->PromoUrl != ''): ?>
									<div class="promoimg promo-red">
										<?php { echo '<a href="'.$promoList[$i]->PromoUrl.'='.$promoList[$i]->PromoCode.'" target="_blank" onclick="promoLog(\''.$promoList[$i]->Title.'\')">'.$promoList[$i]->Summary.'</a>'; } ?>
									</div>
								<!--None-->
								<?php else: ?>
...
function promoLog(title){
	window.location = "<?php echo base_url(); ?>index.php/Common/PromoLog/Deals/"+title;
}
</script>