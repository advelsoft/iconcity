...
class eset extends CI_Controller {
	...
	public function Create()
	{
        ...
        if ($this->form_validation->run() == FALSE)
        {
			...
			$this->load->view('Admin/Setup/Eset/Create', $data);
        }
        else
        {
			//validation succeed
            ...
            redirect('index.php/Common/Promotion/Eset');
        }
    }
	...
	public function Detail($PromoID)
	{
		...
		if($_SESSION['role'] == 'Admin'){
			$this->load->view('Admin/header',$data);
			$this->load->view('Admin/nav');
			$this->load->view('Admin/Setup/Eset/Detail',$data);
			$this->load->view('Admin/footer');
		}
		else if($_SESSION['role'] == 'Mgmt'){
			$this->load->view('Mgmt/header',$data);
			$this->load->view('Mgmt/nav');
			$this->load->view('Mgmt/Promotion/Eset/Detail',$data);
			$this->load->view('Mgmt/footer');
		}
		else{
			$this->load->view('User/header',$data);
			$this->load->view('User/nav');
			$this->load->view('User/Promotion/Eset/Detail',$data);
	...
	public function Update($UID)
	{
        ...
        $data['esetRecord'] = $this->promotion_model->get_eset_record($UID);    
        ...
        {   
			...
			$this->load->view('Admin/nav');
			$this->load->view('Admin/footer');
			$this->load->view('Admin/Setup/Eset/Update', $data);
        }
        else
        {
			//rearrange date
			...
			redirect('index.php/Common/Eset/Update/'.$UID);
        }
	}
	...
	public function Delete($UID)
	{
		...
		$this->load->view('Admin/header',$data);
		$this->load->view('Admin/nav');
		$this->load->view('Admin/Setup/Eset/Delete',$data);
		$this->load->view('Admin/footer');
	}
	...
	public function OrderSumm()
	{
		...
		if($_SESSION['role'] == 'Mgmt'){
			...
			$this->load->view('Mgmt/nav');
			$this->load->view('Mgmt/footer');
			$this->load->view('Mgmt/Promotion/Eset/OrderSumm', $data);
		}
		else{
			$this->load->view('User/header',$data);
			$this->load->view('User/nav');
			$this->load->view('User/footer');
			$this->load->view('User/Promotion/Eset/OrderSumm', $data);
		}
	}
	...
	function OrderRemove($rowid) 
	{
		if ($rowid=="All"){
			...
			redirect('index.php/Common/Eset/OrderSumm');
		}
	}
	...
	public function CheckOut()
	{
		...
		if($_SESSION['role'] == 'Admin'){
			$this->load->view('Admin/header',$data);
			$this->load->view('Admin/nav');
			$this->load->view('Admin/Setup/Eset/PlaceOrder',$data);
			$this->load->view('Admin/footer');
		}
		else if($_SESSION['role'] == 'Mgmt'){
			$this->load->view('Mgmt/header',$data);
			$this->load->view('Mgmt/nav');
			$this->load->view('Mgmt/Promotion/Eset/PlaceOrder',$data);
			$this->load->view('Mgmt/footer');
		}
		else{
			$this->load->view('User/header',$data);
			$this->load->view('User/nav');
			$this->load->view('User/Promotion/Eset/PlaceOrder',$data);
			$this->load->view('User/footer');
		}
	}
	...
	public function PlaceOrder()
	{
        ...
        else
        {
			...
			$header = '<html><head><meta http-equiv="content-type" content="text/html; charset=utf-8"></head>'.
					   '<body dir="auto"><div style="width: 680px;"><img src="'.base_url().'application/uploads/promotion/eset.png" alt="Eset" style="margin-bottom: 20px; border: none;">'.
					   '<p style="margin-top: 0px; margin-bottom: 20px;">Thank you for your interest in Eset products. Your order has been received and will be processed once payment has been confirmed.</p>'.
			...
			{	
				...
				//display success message
				redirect('index.php/Common/Eset/Confirmation/'.$orderid);
			}
			else
			{
				//show_error($this->email->print_debugger());
				//display message
				redirect('index.php/Common/Eset/CheckOut');
			}
        }
	}
	...
	public function sendEmail($config, $message2, $companyName)
	{
		...
		$this->email->subject('New Order - Eset');
	...
	public function Confirmation($orderid)
	{
		...
		if($_SESSION['role'] == 'Admin'){
			$this->load->view('Admin/header',$data);
			$this->load->view('Admin/nav');
			$this->load->view('Admin/Setup/Eset/Confirmation',$data);
			$this->load->view('Admin/footer');
		}
		else if($_SESSION['role'] == 'Mgmt'){
			$this->load->view('Mgmt/header',$data);
			$this->load->view('Mgmt/nav');
			$this->load->view('Mgmt/Promotion/Eset/Confirmation',$data);
			$this->load->view('Mgmt/footer');
		}
		else{
			$this->load->view('User/header',$data);
			$this->load->view('User/nav');
			$this->load->view('User/Promotion/Eset/Confirmation',$data);
			$this->load->view('User/footer');
		}
	}
}?>